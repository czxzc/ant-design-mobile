import init, {
  ...
  setters,
  config,
} from '@alifd/lowcode-preset-plugin'
...
const metaPathMap = {
  'meta': '/Users/zc/Documents/project/ant-design-mobile/.tmp/meta.js',
}
const key = metaType ? `meta.${metaType}` : 'meta'
if (!metaPathMap[key]) {
  console.warn(
    `[@alifd/build-plugin-lowcode] metaType can only in [${Object.keys(
      metaPathMap
    )}], current metaType is ${key}, fullback to ${fullbackMeta || 'default'}.`
  )
...
if (devMode && type !== 'setter') {
  assets.packages.push({
    ...
    'advancedUrls': {
      'default': ['./render/default/view.js', './render/default/view.css'],
    },
  })
        ...
        if (!devMode) {
          ...
          assets.components = devAssets.components.map(item => {
            if (
              item.advancedUrls &&
              metaType &&
              item.advancedUrls[metaType] &&
              item.advancedUrls[metaType].length
            ) {
            ...
            project.onRendererReady(() => {
              editor.get('designer').currentDocument.simulator._iframe.onload =
                () => {
                  editor
                    .get('designer')
                    .currentDocument.simulator.set('device', 'phone')
                }
            })
          }
        }
      ...
      extraAssets.map(async url => {
        if (typeof url === 'object') {
          ...
          try {
            ...
            console.error(
              `get assets data from builtin assets ${url} failed: `,
              e
            )
            return {}
          }
